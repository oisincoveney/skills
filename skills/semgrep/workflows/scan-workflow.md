...
On invocation, track these steps with dependencies in your harness's task system, checklist, or plan tracker:

```
Step 1: Detect languages and Pro availability
Step 2: Select scan mode and rulesets - blocked by Step 1
Step 3: Present plan with rulesets, get approval - blocked by Step 2
Step 4: Execute scans with approved rulesets and mode - blocked by Step 3
Step 5: Merge results and report - blocked by Step 4
```
...
`$OUTPUT_DIR` is used by all subsequent steps. Pass its **absolute path** to scanner workers. Scanners write raw output to `$OUTPUT_DIR/raw/`; merged/filtered results go to `$OUTPUT_DIR/results/`.

**Detect Pro availability** (requires shell command execution):
...
**Detect languages** using file-search tooling before shell fallbacks. Run these patterns against the target directory and count matches:

`**/*.py`, `**/*.js`, `**/*.ts`, `**/*.tsx`, `**/*.jsx`, `**/*.go`, `**/*.rb`, `**/*.java`, `**/*.php`, `**/*.c`, `**/*.cpp`, `**/*.rs`, `**/Dockerfile`, `**/*.tf`

Also check for framework markers: `package.json`, `pyproject.toml`, `Gemfile`, `go.mod`, `Cargo.toml`, `pom.xml`. Read these files for framework dependencies (e.g., read `package.json` to detect React, Express, Next.js; read `pyproject.toml` for Django, Flask, FastAPI).
...
2. Ask directly if the user hasn't responded:
   ```
...
## Step 4: Start Parallel Scan Workers

> **Entry:** Step 3 approved — user explicitly confirmed the plan.
> **Exit:** All scan workers completed; result files exist in `$OUTPUT_DIR/raw/`.
...
Start one scanner worker per language category. If your harness supports parallel workers, start them together. If it does not, run the same scanner prompts sequentially and report that the run was degraded.

Use the scanner prompt template from [scanner-task-prompt.md](../references/scanner-task-prompt.md).
...
Start these 3 workers together when supported:

1. **Python scanner** — Rulesets: p/python, p/django, p/security-audit, p/secrets, trailofbits -> `$OUTPUT_DIR/raw/python-*.json`
2. **JavaScript scanner** — Rulesets: p/javascript, p/react, p/nodejs, p/security-audit, p/secrets, trailofbits -> `$OUTPUT_DIR/raw/js-*.json`
3. **Docker scanner** — Rulesets: p/dockerfile -> `$OUTPUT_DIR/raw/docker-*.json`

### Operational Notes

- Always use **absolute paths** for `[TARGET]` — workers may not resolve relative paths from the same directory
...
> **Entry:** Step 4 complete — all scan workers finished.
