...
This skill is designed for the **main-session orchestrator**, where Step 3 (DOUBT, detailed below) can invoke a fresh-context reviewer.

- **Do NOT attach this skill to a nested-reviewer persona or worker.** A reviewer that follows Step 3 would try to spawn another reviewer. Keep this skill at the orchestration layer.
- **If your harness cannot invoke an isolated reviewer from the current context**, surface that limitation. As a last resort only, use a degraded self-questioning fallback: rewrite ARTIFACT + CONTRACT as a fresh self-prompt with a hard mental separator from your prior reasoning, and walk Steps 1-5. This is **not fresh-context review** because you carry your own context, so label it degraded.
...
Use whatever your harness provides for isolated review: a subagent, another model session, an external CLI, or a human reviewer. The isolation matters more than the tool name.

**The adversarial prompt above takes precedence over the reviewer's default response shape.** General code reviewers often produce balanced verdicts with both strengths and weaknesses; doubt-driven needs issues-only output. Paste the adversarial prompt verbatim into the invocation so it overrides the default. If the default shape can't be overridden cleanly, use a generic fresh-context reviewer with the adversarial prompt.
